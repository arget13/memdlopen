...
_start:
    ...
    read:
        ...
        mov     x8, 0x3f
        svc     0                 // read
        tbnz    x0, 63, err_read  // Error while reading
        add     x20, x20, x0
        cmp     x0, 0x1000
        beq     mremap
    ...
    // Obtain dlopen offset, depending on distro it may be in libc, libdl or ld
        mov     x18, -1           // Counter of libs tried
    next_lib:
        cmp     x18, 2
        // Tried all libs already, just do the right thing and die
        beq     err_nodlopen
        add     x18, x18, 1
        adr     x1, libcpath
        mov     x17, (libdlpath - libcpath)
        madd    x1, x18, x17, x1
        // Map lib in memory first
        mov     x0, -100          // AT_FDCWD
        mov     x2, 0             // O_RDONLY
        mov     x8, 0x38
        svc     0                 // openat
        tbnz    x0, 63, next_lib  // Error opening lib, try next one
        mov     x4, x0
    ...
    next_symbol:
        ...
        add     x9, x9, x5
        bl      strcmp
        cbz     x11, symbol_found
        subs    x1, x1, 0x18
        bne     next_symbol
        b       next_lib           // Couldn't locate dlopen, next lib
    symbol_found:
        ldr     x9, [x4, (-0x18 + 8)]
        adr     x3, data + data.libc_addr
        ldr     x3, [x3, x18, lsl 3]
        add     x3, x3, x9
    ...
    // Call dlopen(FAKE_LIB, RTLD_NOW | RTLD_GLOBAL)
        ...
        cbz     x0, err_loading    // Error during loading, let's cry together
...
err_read:
        ...
        b       _exit
err_loading:
        mov     x2, err_nodlopen_msg - err_load_msg
        adr     x1, err_load_msg
        mov     x0, 1
        mov     x8, 0x40
        svc     0
        b       _exit
err_nodlopen:
        mov     x2, err_read_msg - err_nodlopen_msg
        adr     x1, err_nodlopen_msg
        mov     x0, 1
        mov     x8, 0x40
        svc     0
        b       _exit
...
strcmp:
    ...
    next_byte:
        ...
        cbnz    x12, next_byte     // Still didn't reach end of string
    strcmp_ret:
        ret
...
dlopen    : .asciz "dlopen"
// Extra nullbyte introduced to make libdlpath - libcpath = ldpath - libdlpath
libcpath  : .asciz "/lib/aarch64-linux-gnu/libc.so.6\x00"
libdlpath : .asciz "/lib/aarch64-linux-gnu/libdl.so.2"
ldpath    : .asciz "/lib/aarch64-linux-gnu/ld-linux-aarch64.so.1"

err_load_msg     : .ascii "memdlopen: Error while loading\n"
err_nodlopen_msg : .ascii "memdlopen: Error, could not find dlopen\n"
err_read_msg     : .ascii "memdlopen: Error while reading\n"
...
data.libc_addr     : .space 8
data.libdl_addr    : .space 8
data.ld_addr       : .space 8
